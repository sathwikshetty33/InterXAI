...
Docker: `make provision` is the one-command local setup — it runs `tools/provision.sh` (compose up → migrate → install Piston languages → seed a demo candidate/company + an attendable interview). Plain `docker compose up --build` → api :8000, frontend :8080, piston :2000, redis :6379, **postgres :5433** (local DB, much lower latency than remote Neon; `api`/`taskiq_worker` override `DATABASE_URL`→`postgres:5432` and `DATABASE_SSL=disable`), plus the taskiq worker. `env_file` is `backend/.env` (note: `.env.example` is incomplete — `REDIS_URL`, `GROQ_API_KEY`, `SUPABASE_*`, `LLM_MODEL_NAME` are documented only in the README table). Dev tooling (`provision.sh`, `seed.sh`, `piston_install.sh`) lives in `tools/` so none of it ships in the app image.
...
- **Piston ships with zero language runtimes installed.** A fresh `piston` container/volume answers `GET /api/v2/runtimes` with `[]`, and `/dsa/run|test|submit` then 400 on every language. Language install is **not** in docker-compose — it's `tools/piston_install.sh` (bash, hits the Piston API), run by `tools/provision.sh` (or `make piston`). Already-installed packages are skipped via `/api/v2/packages`'s `installed` flag (keyed by exact package name+version, NOT the runtime language names in `/api/v2/runtimes` — those differ for gcc→c/c++ and mono→csharp), so reruns are a fast no-op. Keep `PACKAGES` in that script in sync with `PistonClient.LANGUAGE_ALIASES` (`app/utils/piston_client.py`) and the editor's language list. Note: `tools/*.sh` run from the **host**, so they use `localhost:2000`/`localhost:8000`; inside docker-compose, services reach each other by name (`http://piston:2000`), which is why `docker-compose.yml` overrides `PISTON_URL` for the containers (`backend/.env`'s value is for running the API directly on the host).
