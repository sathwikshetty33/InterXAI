...
- **Piston ships with zero language runtimes installed.** A fresh `piston` container/volume answers `GET /api/v2/runtimes` with `[]`; `/dsa/run|test|submit` then 400 on every language until runtimes are installed via `POST /api/v2/packages {"language","version"}` (list available ones at `/api/v2/packages`). `PistonClient.LANGUAGE_ALIASES` (`app/utils/piston_client.py`) maps the editor's language ids to Piston runtime names — note C/C++ come from the `gcc` package and C# from `mono`, not packages literally named `c++`/`csharp`. Also: inside docker-compose, `PISTON_URL` must be `http://piston:2000` (the service name), not `localhost:2000` — `backend/.env`'s value is for running the API directly on the host, and `docker-compose.yml` overrides it for the containerized services.
