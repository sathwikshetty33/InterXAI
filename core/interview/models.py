...
class Application(models.Model):
    ...
    shortlisting_decision = models.BooleanField(default=False)  # AI decision
    feedback = models.TextField(blank=True, null=True)
    github_score = models.FloatField(blank=True, null=True)
    # Organization's final decision (manual select/reject)
    final_decision = models.CharField(max_length=20, choices=[('pending', 'Pending'), ('accepted', 'Accepted'), ('rejected', 'Rejected')], default='pending')
    final_feedback = models.TextField(blank=True, null=True)  # Feedback from organization
    def __str__(self):
...
class InterviewImages(models.Model):
    sesssion = models.ForeignKey(InterviewSession, on_delete=models.CASCADE, related_name="images")
    image_url = models.CharField(max_length=255)
    uploaded_at = models.DateTimeField(auto_now_add=True)
    def __str__(self):
        return f'Image for session {self.sesssion.id} at {self.uploaded_at}'