      ...
      <div className="relative z-10 w-full max-w-lg">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-500 text-sm text-center max-w-sm">
            Complete your profile so organisations can find you. All fields are optional — you can update them anytime.
          </p>
        </div>

        {/* Card */}
        <div className="bg-white rounded-2xl shadow-xl shadow-blue-100/50 border border-slate-100 p-8">
          <form id="profile-setup-form" onSubmit={handleSubmit} noValidate className="space-y-5">
            {error && <AlertBanner message={error} variant="error" />}

            {fields.map((f) => (
              <div key={f.id} className="flex flex-col gap-1.5">
                <label htmlFor={f.id} className="text-sm font-medium text-slate-700 flex items-center gap-2">
            ...
            <div className="flex gap-3 pt-2">
              <Button
                ...
                disabled={isLoading}
              >
                {isLoading ? <><Spinner /> Saving…</> : 'Save & Continue'}
              </Button>
              <button
                type="button"
...
function CodeIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      <path d="M4.5 4L1.5 7l3 3M9.5 4l3 3-3 3M8.5 2l-3 10" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
