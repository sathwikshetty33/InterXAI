...
const AvailableCard: React.FC<{ interview: InterviewBasic }> = ({
  interview,
}) => (
  <div
    className="bg-white rounded-2xl border border-slate-100 p-5 hover:shadow-lg hover:shadow-blue-100/50
  ...
  >
    <div className="flex items-start justify-between gap-4 mb-3">
      ...
      <span
        className="shrink-0 text-xs font-medium text-orange-600 bg-orange-50 border border-orange-200
...
const AppliedCard: React.FC<{ interview: AppliedInterview }> = ({
  interview,
}) => {
  const statusClass =
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-5 hover:shadow-lg hover:shadow-blue-100/50 transition-all duration-200">
      <div className="flex items-start justify-between gap-4 mb-3">
        ...
        <span
          className={`shrink-0 text-xs font-semibold border rounded-full px-3 py-1 capitalize ${statusClass}`}
        >
          {interview.status}
        </span>
      </div>
    ...
    <div id="dashboard-page" className="min-h-screen bg-slate-50">
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-slate-200/70 shadow-sm px-6 md:px-12 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          ...
          <div
            className="hidden md:flex items-center gap-2 flex-1 max-w-sm mx-8
        ...
        {!user.profile?.bio && (
          <div className="mb-6 flex items-center gap-4 bg-blue-50 border border-blue-200 rounded-2xl px-5 py-4">
            ...
            <p className="text-sm text-blue-800">
              Your profile is incomplete. Adding your bio and social links helps
              organisations find you.
            </p>
          </div>
        )}
...
const Stat = ({
  ...
  <div
    className={`flex flex-col items-center px-5 py-3 rounded-xl bg-${color}-50 border border-${color}-100`}
  >
...
function BriefcaseIcon() {
  return (
    <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
      ...
      />
      <path
        d="M9 9V7a5 5 0 0110 0v2M3 16h22"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
      />
...
function ClipboardIcon() {
  return (
    <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
      ...
      />
      <path
        d="M10 5a4 4 0 018 0M9 13h10M9 17h6"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
      />
